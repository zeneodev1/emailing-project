        ...
        try {
            ...
            if (TypesParser.safeParseInt(parameters[5]) != 0) {
                ...
                if (new File(logsFolder).exists()) {
                    List<File> filesInFolder = Files.walk(Paths.get(logsFolder, new String[0]), new java.nio.file.FileVisitOption[0]).filter(x$0 -> Files.isRegularFile(x$0, new java.nio.file.LinkOption[0])).map(Path::toFile).collect(Collectors.toList());
